...
def fix_riders_history_file(data_dir: Path, competition: str) -> bool:
    ...
    try:
        ...
        print(
            f"🔧 {competition}: Calculating GC from stage data for {latest_year_all}..."
        )
        ...
        if individual_data.empty:
            print(
                f"⚠️  {competition}: No individual stage data found for {latest_year_all}"
            )
            return False

        print(
            f"📈 {competition}: Found {len(individual_data)} individual stage results"
        )
        ...
        complete_riders = rider_stage_counts[
            rider_stage_counts == max_stages
        ].index.tolist()

        print(
            f"📊 {competition}: {len(complete_riders)} riders completed all {max_stages} stages"
        )
        ...
        print(
            f"🏆 {competition}: Winner: {gc_data.iloc[0]['Rider']} with {gc_data.iloc[0]['TotalSeconds']/3600:.1f}h total time"
        )
        ...
        print(
            f"✅ {competition}: Added {len(new_riders_data)} riders with correct GC times"
        )
        return True
